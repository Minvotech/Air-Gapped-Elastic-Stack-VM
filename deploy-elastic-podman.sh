...
checkcontainer() {
  ...
  do
    if [ $(podman ps -a --format '{{.Names}}' | grep -c ${name}) -ge 1  ]; then
  ...
  do
    if [ $(podman network ls --format '{{.Name}}' | grep -c ${name}) -ge 1 ]; then
  ...
  do
    if [ $(podman volume ls --format '{{.Name}}' | grep -c ${name}) -ge 1 ]; then
...
pullimage() {
  ...
  if [ $? -ne 0 ]; then
    ...
    echo "${green}[DEBUG]${reset} "${1}" podman image already exists.."
  fi
...
cleanup() {
  ...
  podman stop es01 es02 es03 kibana metricbeat filebeat apm entsearch minio01 fleet es_mc_1 es_wait_until_ready_1 apm es_setup_1 es-setup-1 ldap >/dev/null 2>&1
  podman rm es01 es02 es03 kibana metricbeat filebeat apm entsearch minio01 fleet es_mc_1 es_wait_until_ready_1 apm es_setup_1 es-setup-1 ldap >/dev/null 2>&1
  podman network rm es_default >/dev/null 2>&1
  podman volume rm es_data01 >/dev/null 2>&1
  podman volume rm es_data02 >/dev/null 2>&1
  podman volume rm es_data03 >/dev/null 2>&1
  podman volume rm es_fleet >/dev/null 2>&1
  podman volume rm es_certs >/dev/null 2>&1
  podman volume rm es_kibanadata >/dev/null 2>&1
...
EOF
    ...
    # restarting kibana
    podman restart kibana >/dev/null 2>&1
...
volumes: {\"data01\", \"data02\", \"data03\" , \"certs\"}
  "
    ...
    if [ $(checkversion $VERSION) -ge $(checkversion "7.2.0") ]; then
      podman exec es01 /bin/bash -c "cp /usr/share/elasticsearch/config/certificates/ca/ca.* /temp/"
    ...
    if [ $(checkversion $VERSION) -ge $(checkversion "7.6.0") ]; then
      podman exec es01 /bin/bash -c "bin/elasticsearch-setup-passwords auto --batch \
...
EOF
  ...
  if [ $(checkversion $VERSION) -ge $(checkversion "8.0.0") ]; then
    ...
    RMUPASSWD=`podman exec es01 bin/elasticsearch-reset-password -u remote_monitoring_user -a -s -b`
...
EOF
  ...
  IP=`podman inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' minio01`

  # install repository-s3 plugin & add keystore items & restart container
  for instance in es01 es02 es03
  do
    podman exec ${instance} bin/elasticsearch-plugin install --batch repository-s3 >/dev/null 2>&1
    if [ $? -ne 0 ]; then
    ...
    if [ $(checkversion $VERSION) -ge $(checkversion "7.0.0") ]; then
      podman exec -i ${instance} bin/elasticsearch-keystore add -xf s3.client.minio01.access_key <<EOF
minio
EOF
      echo "${green}[DEBUG]${reset} added s3.client.minio01.access_key to keystore to ${instance}"
      podman exec -i ${instance} bin/elasticsearch-keystore add -xf s3.client.minio01.secret_key <<EOF
minio123
EOF
      echo "${green}[DEBUG]${reset} added s3.client.minio01.secret to keystore to ${instance}"
    else
      podman exec ${instance} bash -c "echo '-Des.allow_insecure_settings=true' >> config/jvm.options"
      echo "${green}[DEBUG]${reset} 6.x found adding -Des.allow_insecure_Settings=true in jvm.options"
    fi
    podman restart ${instance} >/dev/null 2>&1
...
EOF
  ...
  echo "${green}[DEBUG]${reset} Started Enterprise Search.  It takes a ${red}WHILE!!${reset} to finish install. Please run podman logs -f entsearch to view progress"
...
- module: enterprisesearch
  ...
  password: "${PASSWD}"
EOF
      podman restart metricbeat >/dev/null 2>&1
...
EOF
  ...
  podman exec ldap ldapadd -x -D "cn=admin,dc=example,dc=org" -w admin -f /tmp/ldap.ldif -H ldap://localhost -ZZ >/dev/null 2>&1
  ...
  curl -k -u elastic:${PASSWD} -XPUT "https://localhost:9200/_security/role_mapping/ldap-user" -H 'Content-Type: application/json' -d'
{
  ...
  for instance in es01 es02 es03
  do
    podman exec -i ${instance} bin/elasticsearch-keystore add -xf xpack.security.authc.realms.ldap.ldap1.secure_bind_password <<EOF
admin
EOF
  podman restart ${instance}  >/dev/null 2>&1
  done
